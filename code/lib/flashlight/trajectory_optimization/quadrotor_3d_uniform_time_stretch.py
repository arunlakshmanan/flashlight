...
def optimize_feasible(p_nominal,psi_nominal,dt_nominal,x_min_ti,x_max_ti,u_min_ti,u_max_ti,max_bin_search_iters,dt_upper_init):
    ...
    print "flashlight.trajectory_optimization.quadrotor_3d_uniform_time_stretch: Attempting to find feasible trajectory with max_bin_search_iters = %d, dt_upper_init = %f." % (max_bin_search_iters,dt_upper_init)
    ...
    q_q_dot_q_dot_dot_lower = quadrotor_3d.compute_state_space_trajectory_and_derivatives(p_nominal,psi_nominal,dt_lower,check_angles=False)
    u_lower                 = quadrotor_3d.compute_control_trajectory(q_q_dot_q_dot_dot_lower)
    ...
    if all(x_lower >= x_min_ti.T) and all(x_lower <= x_max_ti.T) and all(u_lower >= u_min_ti.T) and all(u_lower <= u_max_ti.T):
        print "flashlight.trajectory_optimization.quadrotor_3d_uniform_time_stretch: Input trajectory is already feasible."
        return x_lower,u_lower,1.0

    # assert upper bound satisfies constraints
    q_q_dot_q_dot_dot_upper = quadrotor_3d.compute_state_space_trajectory_and_derivatives(p_nominal,psi_nominal,dt_upper,check_angles=False)
    u_upper                 = quadrotor_3d.compute_control_trajectory(q_q_dot_q_dot_dot_upper)
    ...
    for i in range(max_bin_search_iters):
        ...
        q_q_dot_q_dot_dot_current = quadrotor_3d.compute_state_space_trajectory_and_derivatives(p_nominal,psi_nominal,dt_current,check_angles=False)
...
def optimize_numerically_stable_infeasible(p_nominal,psi_nominal,dt_nominal,max_bin_search_iters,dt_upper_init):
    ...
    print "flashlight.trajectory_optimization.quadrotor_3d_uniform_time_stretch: Attempting to find numerically stable trajectory with max_bin_search_iters = %d, dt_upper_init = %f." % (max_bin_search_iters,dt_upper_init)
    ...
    # if lower bound satisfies constraints, return
    if quadrotor_3d.check_differentially_flat_trajectory_numerically_stable(p_nominal,psi_nominal,dt_lower):
        print "flashlight.trajectory_optimization.quadrotor_3d_uniform_time_stretch: Input trajectory is already numerically stable."
        return 1.0

    # assert upper bound satisfies constraints
    assert quadrotor_3d.check_differentially_flat_trajectory_numerically_stable(p_nominal,psi_nominal,dt_upper)
    ...
    for i in range(max_bin_search_iters):
        ...
        if quadrotor_3d.check_differentially_flat_trajectory_numerically_stable(p_nominal,psi_nominal,dt_current):
            dt_upper = dt_current
        else:
            dt_lower = dt_current
