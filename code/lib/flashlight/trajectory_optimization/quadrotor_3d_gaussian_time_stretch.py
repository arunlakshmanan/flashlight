...
def _get_current_vals(p_eval,psi_eval,t_current,user_progress_current,dt_current):
    ...
    p_eval_user_progress,   t_user_progress, p_eval_cumulative_length,   t_user_progress_linspace_norm = curve_utils.reparameterize_curve( p_eval,   user_progress_current )
    psi_eval_user_progress, t_user_progress, psi_eval_cumulative_length, t_user_progress_linspace_norm = curve_utils.reparameterize_curve( psi_eval, user_progress_current )

    p_current   = p_eval_user_progress
    psi_current = psi_eval_user_progress[:,0]

    q_q_dot_q_dot_dot_current = quadrotor_3d.compute_state_space_trajectory_and_derivatives(p_current,psi_current,dt_current,check_angles=False)
    u_current                 = quadrotor_3d.compute_control_trajectory(q_q_dot_q_dot_dot_current)
...
def _update_easing_curve(t_current,user_progress_current,dt_current,num_timesteps,t_begin,gauss_mean_in_terms_of_t,gauss_width_in_terms_of_dt,gauss_max_in_terms_of_dt):
    ...
    user_progress_current = interpolate_utils.resample_scalar_wrt_scalar(t_stretched,user_progress_current,t_current)
...
def optimize_feasible(p_eval,psi_eval,t_nominal,user_progress_nominal,dt_nominal,x_min_ti,x_max_ti,u_min_ti,u_max_ti,max_stretch_iters,gauss_width_in_terms_of_dt,gauss_max_in_terms_of_dt,extra_iters):
    ...
    print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Attempting to find feasible trajectory with max_stretch_iters = %d." % max_stretch_iters
    ...
    for i in range(max_stretch_iters):
        ...
        if all(x_current >= x_min_ti.T) and all(x_current <= x_max_ti.T) and all(u_current >= u_min_ti.T) and all(u_current <= u_max_ti.T):
            print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Found feasible trajectory after %d smoothing iterations." % i
            found = True
            break
        else:
    ...
    if found:
        ...
        print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Applying %d extra smoothing iterations to feasible trajectory." % extra_iters
        ...
        print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Could not find feasible trajectory."
        return None
...
def optimize_numerically_stable_infeasible(p_eval,psi_eval,t_nominal,user_progress_nominal,dt_nominal,x_min_ti,x_max_ti,u_min_ti,u_max_ti,max_stretch_iters,gauss_width_in_terms_of_dt,gauss_max_in_terms_of_dt,max_extra_iters):
    ...
    print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Attempting to find numerically stable trajectory with max_stretch_iters = %d." % max_stretch_iters
    ...
    for i in range(max_stretch_iters):
        ...
        if quadrotor_3d.check_differentially_flat_trajectory_numerically_stable(p_current,psi_current,dt_current):
            print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Found numerically stable, but not neccesarily feasible, trajectory after %d smoothing iterations." % i
            found = True
            break
        else:

            udotN_current                              = gradient_utils.gradients_vector_wrt_scalar_smooth_boundaries(u_current,dt=dt_current,max_gradient=2,poly_deg=5)
    ...
    if found:
        ...
        print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Applying at most %d extra smoothing iterations to numerically stable trajectory." % max_extra_iters
        ...
        for i in range(max_extra_iters):
            ...
            if all(x_current >= x_min_ti.T) and all(x_current <= x_max_ti.T) and all(u_current >= u_min_ti.T) and all(u_current <= u_max_ti.T):
                print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Found feasible trajectory after %d extra smoothing iterations to numerically stable trajectory." % i
                break
            else:
                t_umax                                     = t_current[ unravel_index( argmax(abs(u_current)), u_current.shape )[0] ]
                gauss_mean_in_terms_of_t                   = t_umax
        ...
        print "flashlight.trajectory_optimization.quadrotor_3d_gaussian_time_stretch: Could not find numerically stable trajectory."
        return None
